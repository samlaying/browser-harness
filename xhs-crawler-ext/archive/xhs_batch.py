...
# 收集阶段的滚动快照（run_batch 设置，scroll_to_card 使用）
COLLECT_SCROLL_Y = 0       # 收集时 window.scrollY
CARD_Y_MAP = {}            # note_id → 收集时的 viewport-relative y
...
''' % note_id)
    if found:
        return
    # 卡片不在 DOM → 滚动到收集时的近似位置触发虚拟滚动渲染
    est_y = CARD_Y_MAP.get(note_id)
    if est_y is not None:
        est_doc_y = est_y + COLLECT_SCROLL_Y
...
def run_batch():
    ...
    # 记录收集时的滚动快照，供 scroll_to_card 在虚拟滚动时估算文档位置
    global COLLECT_SCROLL_Y, CARD_Y_MAP
    COLLECT_SCROLL_Y = safe_js('return window.scrollY;') or 0
    CARD_Y_MAP = {c['id']: c['y'] for c in cards}
    # 注意：不再 scrollTo(0,0)，卡片在滚动加载后收集，需逐个 scroll_to_card
